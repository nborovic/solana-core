...
export class StudentIntro {
  ...
  serialize(): Buffer {
    const tempBuffer = Buffer.alloc(1000);
    this.borshInstructionSchema.encode({ ...this, variant: 0 }, tempBuffer);
    return tempBuffer.subarray(
      0,
      this.borshInstructionSchema.getSpan(tempBuffer)
    );
  }
}
